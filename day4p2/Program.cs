...
{
    if (IsBeetweenValues(ranges[i, 0], ranges[i, 2], ranges[i, 3]) || IsBeetweenValues(ranges[i, 1], ranges[i, 2], ranges[i, 3]) || IsBeetweenValues(ranges[i, 2], ranges[i, 0], ranges[i, 1]) || IsBeetweenValues(ranges[i, 3], ranges[i, 0], ranges[i, 1]))
    {
...
static bool IsBeetweenValues(int n, int n1, int n2)
{
    if (n >= n1 && n <= n2)
    {
        return true;
